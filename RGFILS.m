...
function U =RGFILS(F, lambda, p, eps, iter)
...
for k = 1: iter
    ...
    for i=1:2
        for char=1:char
        u_h(:,:,char)= bilateralFilter(u_h(:,:,char), u_h(:,:,char), min(u_h(:)), max(u_h(:)), 10, 0.008); %10 0.008
        u_v(:,:,char)= bilateralFilter(u_v(:,:,char), u_v(:,:,char), min(u_v(:)), max(u_v(:)), 10, 0.008);
        end

    end
    %figure,imshow([u_h,u_v]);

    u_h=gpuArray(single(u_h));
    u_v=gpuArray(single(u_v));
    %%
        
    mu_h = c .* u_h - p .* u_h .* (u_h .* u_h + eps) .^ gamma;
    mu_v = c .* u_v - p .* u_v .* (u_v .* u_v + eps) .^ gamma;
    %figure,imshow([mu_h,mu_v]);
...
%     % Update the smoothed image U
    ...
    %figure,imshow([Normin2_h,Normin2_v]);

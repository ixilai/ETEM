...
chosen=43;
for ii=43:chosen
    img1 = imread(['E:\图像融合\医学图像实验\CT-MRI论文所用图像\CT\new\',num2str(ii),'.PNG']);
    img2 = imread(['E:\图像融合\医学图像实验\CT-MRI论文所用图像\MRI\new\',num2str(ii),'.PNG']);
...
s=3;    r=0.05;   N=4;    T=21; 
%% image decomposition
lambda =3;   npad = 7;  %3 7  lambda取3 未放出来
...
[~, min_gr_ir, max_gr_ir,max_min_ir] = LCP(S1,3);  %窗口大小参数讨论 平常实验 3
...
fuse_High=mapp2.*S1+~mapp2.*S2;  
 ...
 %%
% imwrite(fuse_High,['E:\图像融合\医学图像实验\0 医学 灰度+彩色 实验结果\CT-MRI\LXL论文所用图像2\fuse_High\20009.tif']);
%%
...
%  imwrite(fused_low,['E:\图像融合\医学图像实验\0 医学 灰度+彩色 实验结果\CT-MRI\LXL论文所用图像2\Lowf\16011fused_low.tif']);
%  imwrite(IF,['E:\图像融合\医学图像实验\0 医学 灰度+彩色 实验结果\CT-MRI\LXL论文所用图像2\Lowf\16011IF.tif']);
%%
...
W2=double(W2);
FW=double(FW);
 %imwrite(IF,['E:\图像融合\医学图像实验\论文图像\IF.tif']);
% imwrite(W1,['E:\图像融合\医学图像实验\0 医学 灰度+彩色 实验结果\CT-MRI\LXL论文所用图像2\wenli\20009w1.tif']);
% imwrite(W2,['E:\图像融合\医学图像实验\0 医学 灰度+彩色 实验结果\CT-MRI\LXL论文所用图像2\wenli\20009w2.tif']);
% imwrite(FW,['E:\图像融合\医学图像实验\0 医学 灰度+彩色 实验结果\CT-MRI\LXL论文所用图像2\wenli\20009FW.tif']);
%imwrite(F,['E:\图像融合\医学图像实验\0 医学 灰度+彩色 实验结果\CT-MRI\LXL论文所用图像2\Lowf\16011F.tif']);



%figure,imshow(D,[]); figure,imshow([F,FF]);
% imwrite(F,['E:\图像融合\多聚焦实验\错误纹理去除\', num2str(ii),'.tif']);
%imwrite(IF,['E:\图像融合\医学图像实验\消融实验\纹理去除前\', num2str(ii),'.tif']);
  close all
 end